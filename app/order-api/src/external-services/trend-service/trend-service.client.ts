...
// Client wrapper goi HTTP noi bo sang trend (/internal/*). Cac module khac
// khong tu dung HttpService truc tiep - luon qua wrapper nay.
@Injectable()
export class TrendServiceClient {
  constructor(
  ...
  // khop chinh xac voi path that su ma InternalApiGuard cua trend nhan duoc
  // (request.originalUrl), neu khong chu ky HMAC se sai.
  private async post<T>(relativePath: string, body: unknown): Promise<T> {
    const baseUrl =
      this.configService.get<string>('TREND_API_URL') ||
      'http://localhost:8087';
